...
class TiagoEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: TiagoEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.reference_signals_list = [[[] for _ in range(self.num_dofs)] for _ in range(self.num_envs)]
        self.actual_velocities_list = [[[] for _ in range(self.num_dofs)] for _ in range(self.num_envs)]
        self.errors_list = [[[] for _ in range(self.num_dofs)] for _ in range(self.num_envs)]
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        #self.time_interval = time.time() - self.previous_time
