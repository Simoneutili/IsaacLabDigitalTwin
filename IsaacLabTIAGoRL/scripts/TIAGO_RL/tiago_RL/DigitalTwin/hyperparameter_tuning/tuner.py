...
def invoke_tuning_run(cfg: dict, args: argparse.Namespace) -> None:
    ...
    searcher = OptunaSearch(
        metric=args.metric,  # Make sure this matches the name reported in step()
        mode=args.mode,
    )

    print(f"[DEBUG]: OptunaSearch initialized with metric: {searcher.metric} and mode: {searcher.mode}")
    ...
    print("[INFO]: Starting Tune with tuner.fit()...")
    results = tuner.fit()
    print("[INFO]: Tune execution completed!")

    # ✅ Check Optuna's best trials
    best_trial = results.get_best_result(metric=args.metric, mode=args.mode)
    print(f"[DEBUG]: Best trial found by Optuna: {best_trial.metrics}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metric", type=str, default="rollout/ep_rew_mean", help="What metric to tune for.")
